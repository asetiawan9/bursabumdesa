...
if(isset($prjt_id1)) {
	if(isset($_FILES['img1']['name'])) {
		...
		$db_oldimg = $db->extractCol("select img1 from project where id='$prjt_id1'");
		if($db_oldimg != 'noimage.jpg')
			$oldimg = $db_oldimg;
		else
			$oldimg = '';
		...
		?>
			<img class="comment-avatar" alt="" src="<?=dirname($baseUrl);?>/uploads/prjt-img/img1/<?=$currimg;?>" style="height:150px;width:150px;" />
...
if(isset($prjt_id2)) {
	if(isset($_FILES['img2']['name'])) {
		...
		?>
			<img class="comment-avatar" alt="" src="<?=dirname($baseUrl);?>/uploads/prjt-img/img2/<?=$currimg;?>" style="height:150px;width:150px;" />
...
if(isset($prjt_id3)) {
	...
	if(isset($_FILES[$img_name]['name'])) {
		...
		?>
			<img class="comment-avatar" alt="" src="<?=dirname($baseUrl);?>/uploads/prjt-img/img3/<?=$currimg;?>" style="height:150px;width:150px;" />
...
if(isset($prjt_id4)) {
	...
	if(isset($_FILES[$img_name]['name'])) {
		...
		if($err_img=='ok') {
			...
			$currimg = $db->extractCol("select $img_indb from project where id='$prjId'");
		?>
			<img class="comment-avatar" alt="" src="<?=dirname($baseUrl);?>/uploads/prjt-img/img4/<?=$currimg;?>" style="height:150px;width:150px;" />
	...
	if(isset($_FILES[$txt_name]['name'])) {
		...
		?>
			<video width="300" height="200" controls>
				<source src="<?=dirname($baseUrl);?>/uploads/video/<? echo $currfile; ?>" type="video/mp4">
			</video>
